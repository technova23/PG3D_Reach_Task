...
class ReachDatasetConfig:
    ...
    use_trajectory_family_conditioning: bool = True
...
class ReachSequenceDataset(torch.utils.data.Dataset):
    ...
    def shape_meta(self) -> dict[str, dict[str, dict[str, list[int]]]]:
        ...
        family_dim = None
        if self._use_family_conditioning():
            family_dim = int(self.root["data"]["trajectory_family_onehot"].shape[1])
        return reach_shape_meta(
            num_points=int(point_cloud.shape[1]),
            point_dim=int(point_cloud.shape[2]),
            state_dim=int(state.shape[1]),
            action_dim=int(action.shape[1]),
            trajectory_family_dim=family_dim,
        )
    ...
    def __getitem__(self, idx: int) -> DP3Batch:
        ...
        if self._use_family_conditioning():
            obs["trajectory_family_onehot"] = torch.from_numpy(
                sample["trajectory_family_onehot"].astype(np.float32)
            )
        return {
            "obs": obs,
    ...
    def _validate_arrays(self) -> None:
        ...
        if self._use_family_conditioning():
            family = data["trajectory_family_onehot"]
            if family.shape[0] != total_steps:
                raise ValueError(
                    f"/data/trajectory_family_onehot length {family.shape[0]} != total steps {total_steps}"
                )
            if family.ndim != 2:
                raise ValueError("/data/trajectory_family_onehot must have shape [T, family_dim]")
    ...
    def _use_family_conditioning(self) -> bool:
        return (
            bool(self.config.use_trajectory_family_conditioning)
            and "trajectory_family_onehot" in self.root["data"]
        )
    ...
    def _sample_sequence(self, idx: int) -> dict[str, np.ndarray]:
        ...
        if self._use_family_conditioning():
            keys.append("trajectory_family_onehot")
        keys = tuple(keys)
        sample = {
...
def reach_shape_meta(
    *,
    ...
    state_dim: int = 9,
    action_dim: int = 7,
    trajectory_family_dim: int | None = None,
    ...
    if trajectory_family_dim is not None and trajectory_family_dim > 0:
        obs["trajectory_family_onehot"] = {"shape": [trajectory_family_dim]}
    return {
        "obs": obs,
