...
class DP3Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.goal_key = "goal_xyz"
        self.ee_key = "ee_position"
        self.goal_rel_key = "goal_rel"
        ...
        self.log_goal_encoder_shapes = bool(log_goal_encoder_shapes)
        self._logged_goal_encoder_shapes = False
        if self.goal_marker_points < 0:
            raise ValueError("goal_marker_points must be non-negative")
        if goal_encoder_output_dim <= 0:
            raise ValueError("goal_encoder_output_dim must be positive")
        ...
        if self.use_goal_encoder:
            for key in (self.goal_key, self.ee_key):
                if tuple(observation_space.get(key, ())) != (3,):
                    raise ValueError(f"{key} observation must have shape [3]")
        ...
        if self.use_goal_encoder:
            self.goal_encoder = nn.Sequential(
                nn.Linear(3, 64),
                nn.ReLU(),
                nn.Linear(64, goal_encoder_output_dim),
                nn.ReLU(),
            )
        else:
        ...
        self.n_output_channels = (
            ...
            + (goal_encoder_output_dim if self.use_goal_encoder else 0)
        )
    ...
    def forward(self, observations: Mapping[str, torch.Tensor]) -> torch.Tensor:
        ...
        if self.use_goal_encoder:
            if self.goal_rel_key in observations:
                goal_rel = observations[self.goal_rel_key]
            else:
                goal_rel = observations[self.goal_key] - observations[self.ee_key]
            assert self.goal_encoder is not None
            goal_feat = self.goal_encoder(goal_rel)
            if self.log_goal_encoder_shapes and not self._logged_goal_encoder_shapes:
                print(f"goal_xyz.shape={tuple(observations[self.goal_key].shape)}")
                print(f"goal_rel.shape={tuple(goal_rel.shape)}")
                print(f"goal_feat.shape={tuple(goal_feat.shape)}")
            features.append(goal_feat)

        fused_feat = torch.cat(features, dim=-1)
        if self.use_goal_encoder and self.log_goal_encoder_shapes and not self._logged_goal_encoder_shapes:
            print(f"fused_feat.shape={tuple(fused_feat.shape)}")
            self._logged_goal_encoder_shapes = True
        return fused_feat
