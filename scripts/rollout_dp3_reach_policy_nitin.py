...
def policy_action_to_sim_action(
    ...
    *,
    ...
    if action.shape[0] != 8:
        raise ValueError(f"policy action must have shape [8], got {action.shape}")
    if state.shape[0] < 9:
        raise ValueError(f"current state must have at least 9 values, got {state.shape}")
    if action_mode == "abs_joint":
        arm_action = action
    elif action_mode == "delta_joint":
        arm_action = state[:8] + action
    else:
        raise ValueError(f"unsupported action_mode {action_mode!r}")

    if sim_action_dim == 8:
        sim_action = arm_action.astype(np.float32, copy=True)
    elif sim_action_dim == 7:
