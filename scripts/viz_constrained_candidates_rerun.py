...
CandidateNoiseMode = Literal["random", "seeded"]
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        episode_results = _evaluate_episode_range(
            args=args,
            env=env,
            policy=policy,
    ...
    total_sampled = sum(result["sampled_count"] for result in episode_results)
    total_successful = sum(len(result["successful_candidates"]) for result in episode_results)
    total_plotted = sum(len(result["candidates"]) for result in episode_results)
    print(
        "writing all sampled policy candidates: "
        f"episodes={len(episode_results)} "
        f"plotted={total_plotted}/{total_sampled} "
        f"successful={total_successful}/{total_sampled}",
        flush=True,
    )
    _write_rerun(
        rr=rr,
    ...
    if args.video is not None:
        if len(episode_results) == 1:
            _write_matplotlib_video(
                output=args.video,
                initial_entry=episode_results[0]["initial_entry"],
                candidate_paths=[candidate["tcp_path"] for candidate in episode_results[0]["candidates"]],
                candidate_summaries=episode_results[0]["candidates"],
                fps=args.video_fps,
            )
        else:
            for episode_result in episode_results:
                output = args.video.with_name(
                    f"{args.video.stem}_episode_{episode_result['episode_index']:03d}{args.video.suffix}"
                )
                _write_matplotlib_video(
                    output=output,
                    initial_entry=episode_result["initial_entry"],
                    candidate_paths=[candidate["tcp_path"] for candidate in episode_result["candidates"]],
                    candidate_summaries=episode_result["candidates"],
                    fps=args.video_fps,
                )
    ...
    summary = {
        ...
        "candidate_noise_mode": args.candidate_noise_mode,
        "sampled_count": total_sampled,
        "plotted_count": total_plotted,
        "successful_count": total_successful,
        "search_episodes": args.search_episodes,
        "search_start_episode_index": args.episode_index,
        "search_results": [episode["search_row"] for episode in episode_results],
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="random",
        help=(
            "how to seed the diffusion latent for each candidate; "
            "'random' uses fresh entropy per candidate, 'seeded' is reproducible"
        ),
    )
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("./nitin/spline_include_failed_also.rrd"),
    )
    parser.add_argument(
        "--video",
        type=str,
...
def _candidate_noise_seed(
    *,
    args: argparse.Namespace,
    episode_index: int,
    candidate_index: int,
) -> int:
    if args.candidate_noise_mode == "seeded":
        return int(args.seed + episode_index * args.candidates + candidate_index)
...
def _resolve_episode_seeds(metadata: dict[str, Any]) -> list[int]:
...
def _evaluate_episode_range(
    *,
    ...
    if not episode_results:
        raise RuntimeError("episode search produced no candidate results")
    if any(len(result["successful_candidates"]) < args.min_successful_candidates for result in episode_results):
        print(
            "warning: one or more searched episodes have fewer successful candidates than requested",
            file=sys.stderr,
        )
...
def _evaluate_episode_candidates(
    *,
    ...
    for idx in range(args.candidates):
        noise_seed = _candidate_noise_seed(
            args=args,
            episode_index=episode_index,
            candidate_index=idx,
        )
    ...
    *,
    ...
    while steps < max_steps:
        ...
        if fixed_replan_seed is not None:
            ...
            sampler_generator.manual_seed(fixed_replan_seed)
        with torch.no_grad():
            ...
            action_chunk = _predict_action_with_generator(
                policy=policy,
    ...
    *,
    ...
    for episode_result in episode_results:
        ...
        if scene_points.size:
            rr.log(
                f"episodes/{episode_index:03d}/point_cloud",
        ...
        rr.log(f"episodes/{episode_index:03d}/goal", rr.Points3D(target, colors=[0, 255, 0], radii=0.018))
        rr.log(
            f"episodes/{episode_index:03d}/start_tcp",
            rr.Points3D(start, colors=[255, 220, 0], radii=0.014),
        )
        for idx, candidate in enumerate(candidates):
            path = np.asarray(candidate["tcp_path"], dtype=np.float32)
            if path.shape[0] < 2:
                continue
            success = bool(candidate.get("success", False))
            color = _candidate_cluster_color(
                candidate,
                dim_failed=True,
                failed_color=(255, 60, 60),
            )
            cluster_id = int(candidate.get("trajectory_cluster_id", -1))
            cluster_name = str(candidate.get("trajectory_cluster_name", "unknown"))
            prefix = "success" if success else "failed"
            rr.log(
                f"episodes/{episode_index:03d}/candidates/{cluster_id:02d}_{cluster_name}/{prefix}_{idx:02d}",
                rr.LineStrips3D([path], colors=color),
                static=True,
            )
            rr.log(
                f"episodes/{episode_index:03d}/candidates/{cluster_id:02d}_{cluster_name}/end_{idx:02d}",
...
def _candidate_cluster_color(
    ...
    *,
    dim_failed: bool,
    failed_color: tuple[int, int, int] = (255, 80, 80),
) -> tuple[int, int, int]:
    cluster_id = int(candidate.get("trajectory_cluster_id", -1))
    ...
    if not bool(candidate.get("success", False)):
        return failed_color
    return _trajectory_cluster_color(cluster_id)
