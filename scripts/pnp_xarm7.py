...
* ``scene``  -- STAGE 1 (this one). Reset the env a few times and save a
  ...
  looked at first.
* ``pick``   -- STAGE 2 (this one, now implemented). The trained DP3 reach
  ...
  no gripper output at all, using ``pg3d.envs.xarm_adapter.agents.XArm7Gripper``
  as-is -- no separate gripper-tuning stage here. That class's
  ``gripper_force_limit`` is already a first-principles derivation (see its
  docstring for the fingertip-force sizing math) and its four-bar
  loop-closure fix keeps the fingers genuinely parallel under load, so
  there is no untuned gripper physics left to sweep the way an earlier
  branch's ``tune-gripper`` mode needed to.
* ``pick-place`` -- STAGE 3 (implemented). Everything ``pick`` does, then
...
def _quat_angular_distance_rad(q1_wxyz: np.ndarray, q2_wxyz: np.ndarray) -> float:
    """Angle (radians) between two orientations, quaternion double-cover safe."""
...
def _set_site_pose(env: Any, site_name: str, position: np.ndarray) -> None:
    ...
    ManiSkill's BATCHED ``Pose.create_from_pq(<(1,3) array>)``, not a raw
    ``sapien.Pose(p=...)`` -- on this GPU-backed sim a raw sapien.Pose does
    not reliably land in the CUDA rigid-body buffer, so the actor silently
    keeps its old pose.
    """
...
def run_pick(args: argparse.Namespace, *, place_enabled: bool = False) -> int:
    """STAGE 2 (pick) / STAGE 3 (pick-place), sharing one implementation.

    Deliberately NOT policy-steered yet and deliberately NOT orientation-
    aware: every approach is straight down (``_DOWN_QUAT_WXYZ``), the same
    grasp shape ``pg3d.envs.xarm_adapter.agents.XArm7Gripper`` was already
    tuned for. This stage's own addition is that the APPROACH is the trained
    DP3 reach checkpoint, steered by the same reranking mechanism the reach
    evals use
    ...
    2. SCRIPTED VERTICAL DESCENT onto the cube (straight down == the approach
       ...
       the cube.
    3. SCRIPTED CLOSE (ramped over ``--close-steps``) then LIFT, using
       ``XArm7Gripper``'s own tuned settings (agents.py) directly. The
       checkpoint has no gripper output at all, so close/lift can only ever
       be scripted -- that is not a shortcut, it is the checkpoint's actual
       action space.

    With ``place_enabled`` (STAGE 3, ``--mode pick-place``), a successful pick
    continues into:
            ...
            if rest_gap is None:
                ...
                if rest_gap < args.cube_edge:
                    print(
                        ...
                        "Check --cube-edge against the gripper's real opening.",
                        file=sys.stderr,
                    )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="xArm7 pick-and-place, staged: scene -> pick -> place."
    )
    p.add_argument(
        "--mode",
        default="scene",
        choices=["scene", "pick", "pick-place"],
        help="Which stage to run: scene -> pick -> pick-place. No tune-gripper "
        "mode here -- this branch's gripper (agents.py's XArm7Gripper) is "
        "already tuned, with a first-principles force_limit derivation and "
        "a four-bar loop-closure fix keeping the fingers genuinely parallel "
        "under load; re-sweeping it isn't needed the way it was on a branch "
        "without that fix.",
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=0.07,
        help="Cube side length in metres. 0.07 (7cm) matches "
        "eval_envs/pp_eval.py's own CUBE_HALF_SIZE=0.035 exactly -- this "
        "branch's gripper (four-bar loop closure, see agents.py) does not "
        "need the cube shrunk the way an earlier branch's did.",
    )
    p.add_argument(
        "--goal-marker-radius",
        type=float,
    ...
    p.add_argument("--pregrasp-height", type=float, default=0.12, help="pick: standoff above the cube (m).")
    p.add_argument("--lift-height", type=float, default=0.15, help="pick: vertical lift (m).")
    p.add_argument("--close-steps", type=int, default=15, help="pick: steps to ramp the gripper shut.")
    p.add_argument(
        "--gripper-open-value", type=float, default=0.0,
        help="Raw drive_joint target, radians (0.0 = open). This branch's "
        "gripper controller sets normalize_action=False (agents.py), so "
        "this is spliced into the action vector as-is, not through a "
        "[-1, 1] normalized range.",
    )
    p.add_argument(
        "--gripper-close-value", type=float, default=0.84,
        help="Raw drive_joint target, radians -- matches "
        "XArm7Gripper._GRIPPER_CLOSED (0.85 minus the limit margin) "
        "exactly. The joint never reaches this on a real cube -- it "
        "stalls at the object's half-width and gripper_force_limit "
        "decides how hard it holds there (see that class's docstring).",
    )
    p.add_argument("--move-max-steps", type=int, default=120, help="pick: replay budget per planned move.")
    p.add_argument(
        "--success-lift-fraction",
        type=float,
        default=0.5,
        help="pick: cube must rise this fraction of --lift-height to count as held.",
    )
    p.add_argument("--video-dir", type=Path, default=None, help="pick: directory to record videos into.")
    ...
    p.add_argument(
        ...
        "(default: leave the class default from agents.py -- this branch's "
        "own already-derived 1.0, see that class's docstring for the "
        "fingertip-force sizing math).",
    )
