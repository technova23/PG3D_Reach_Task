...
Post-success hold accounting now uses the logged TCP-to-goal distance and resets immediately when
it exceeds the configured 2.5 cm threshold. `termination_reason=stable_success` therefore requires
the initial in-threshold sample plus 16 consecutive in-threshold hold samples, and
`stable_goal_reached` can recognize a later complete hold after an earlier hit-and-drift. Episode
rows expose current and maximum hold counts plus first goal-entry and stable-streak entry steps.
This fixes the inconsistency exposed by the max-300 hybrid run,
